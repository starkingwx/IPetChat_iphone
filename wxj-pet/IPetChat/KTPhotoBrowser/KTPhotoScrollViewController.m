...
- (void)dealloc 
{
   ...
   [dataSource_ release], dataSource_ = nil;
    [photodescribe_ release],photodescribe_ = nil;
...
- (void)loadView 
{
   ...
   UIBarButtonItem *trashButton = nil;
    if ([[NSUserDefaults standardUserDefaults] integerForKey:@"wherefrom"] == 0) {
...
- (void)viewWillAppear:(BOOL)animated 
{
   ...
   [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleBlackTranslucent animated:YES];
    ...
    if ([[NSUserDefaults standardUserDefaults] integerForKey:@"wherefrom"] == 0) {
        UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithTitle:@"设为封面" style:UIBarButtonItemStyleDone target:self action:@selector(setascover)];
...
-(void)setascover{
    ...
    NSDictionary *dict = [array objectAtIndex:currentIndex_];
    Enhttpmanager *http = [[Enhttpmanager alloc]init];
    [http modifygalleryinfo:self selector:@selector(modifygallerycallback:) galleryid:[[dict objectForKey:@"galleryid"]intValue] coverurl:[dict objectForKey:@"path"]];
    [http release];
}
...
-(void)getgallerylist:(NSArray*)args{
    if ([[args objectAtIndex:0] intValue] == PORTAL_RESULT_GET_GALLERYLIST_SUCCESS) {
        ...
        if ([[dict objectForKey:@"result"] intValue] == 0) {
            NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
            [defaults setObject:[dict objectForKey:@"list"] forKey:@"refreshlist"];
            [defaults setBool:true forKey:@"refresh"];
...
- (CGRect)frameForPageAtIndex:(NSUInteger)index 
{
   ...
   pageFrame.size.width -= (2 * PADDING);
    pageFrame.size.height = bounds.size.height-44;
   pageFrame.origin.x = (bounds.size.width * index) + PADDING;
    
    photodescribe_ = [[UILabel alloc]initWithFrame:CGRectMake(150+pageFrame.origin.x, 436, 320, 44)];
    photodescribe_.backgroundColor = [UIColor blackColor];
    photodescribe_.textColor = [UIColor whiteColor];
    photodescribe_.text = [[[[NSUserDefaults standardUserDefaults] objectForKey:@"photosinfoarray"]objectAtIndex:index] objectForKey:@"description"];
    [scrollView_ addSubview:photodescribe_];
   return pageFrame;
}
...
- (void)trashPhoto 
{
   UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:nil
                                              ...
                                              destructiveButtonTitle:NSLocalizedString(@"删除照片", @"Delete Photo button text.")
